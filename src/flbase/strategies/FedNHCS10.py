...
class FedNHCS10Client(FedUHClient):
    def __init__(self, criterion, trainset, testset, client_config, cid, device, **kwargs):
        ...
        self.score = - np.sum(self.p * (np.log(self.p)))
        self.score2 = np.sum(np.array(temp)) * self.score
        print(self.score)
        print(self.score2)
    ...
    def upload(self):
        if self.client_config['FedNH_client_adv_prototype_agg']:
            ...
            return self.new_state_dict, self._estimate_prototype(), (self.score, self.score2)


class FedNHCS10Server(FedUHServer):
    def __init__(self, server_config, clients_dict, exclude, **kwargs):
        ...
        self.sort_client_ref2 = np.zeros(shape=(10, 10), dtype=int)
    ...
    def aggregate(self, client_uploads, round):
        ...
        if round <= 10:
            ...
            for idx, (client_state_dict, prototype_dict, ref_score) in enumerate(client_uploads):
                ...
                ref_list.append(ref_score[0])
                ref2_list.append(ref_score[1])
            ...
            print("---check ref2_list : ", ref2_list)
            sorted_ref2 = np.array([x for _, x in sorted(zip(ref2_list, self.active_clients_indicies), reverse=True)])
            print("---check sorted_ref2 : ", sorted_ref2)
            self.sort_client_ref2[round - 1] = sorted_ref2
        ...
        if round == 10:
            ...
            self.sort_client_ref2 = self.sort_client_ref2.transpose()
            print("---check self.sort_client_ref2 : ")
            print(self.sort_client_ref2)
        ...
        for r in round_iterator:
            ...
            if r <= 10:
                ...
                if residual == 1:
                    ...
                    clients_consider = self.sort_client[0]
                    print("client consider :", clients_consider)
                self.active_clients_indicies = clients_consider[10*(residual-1):10*residual] if residual > 0 else clients_consider[-10:]
            else:
