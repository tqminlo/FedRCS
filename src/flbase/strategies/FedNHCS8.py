...
class FedNHCS8Client(FedUHClient):
    def __init__(self, criterion, trainset, testset, client_config, cid, device, **kwargs):
        ...
        self.num_sample = np.sum(np.array(temp).astype(int))
        print(self.score)
    ...
    def upload(self):
        if self.client_config['FedNH_client_adv_prototype_agg']:
            ...
            return self.new_state_dict, self._estimate_prototype(), (self.score, self.num_sample)


class FedNHCS8Server(FedUHServer):
    ...
    def aggregate(self, client_uploads, round):
        ...
        if round <= 10:
            ...
            for idx, (client_state_dict, prototype_dict, ref_score) in enumerate(client_uploads):
                ...
                p = (acc_per_class / count_per_class_test + 1e-12).cpu().numpy()
                score = - np.sum(p * (np.log(p)))
            ...
            sorted_idx = np.array([x for _, x in sorted(zip(score_list, self.active_clients_indicies), reverse=True)])
