...
class FedAvgCS10Client(Client):
    ...
    def upload(self):
        return self.new_state_dict, self.count_by_class_full
...
class FedAvgCS10Server(Server):
    ...
    def aggregate(self, client_uploads, round):
        ...
        if round <= 10:
            acc_list = []
            score_list = []
            for idx, (client_state_dict, count_per_class_client) in enumerate(client_uploads):
        ...
        with torch.no_grad():
            for idx, (client_state_dict, _) in enumerate(client_uploads): 
